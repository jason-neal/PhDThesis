...
\begin{table*}
    ...
    \begin{threeparttable}[b]
        \caption[List of {CRIRES} observations.]{Details regarding each of the {CRIRES} observations.
            The number of artefacts removed in \cref{subsubsec:reductionartefacts} as well as the \snr{} of the combined spectra is provided.
            The last three columns are the {RV} calculated from the orbital solution for both the host and largest companion, and  the {RV} difference between them.}
        %\begin{tabular}{l c c c c cl cl r@{.}l r@{.}l r@{.}l}
        \begin{tabular}{l c c c c c c | r@{.}l r@{.}l r@{.}l}
            \toprule
            Object & Obs.& Start date  & Filter & Airmass  & Artefacts & \snr{} & \multicolumn{2}{c}{\({RV}_{1}\)} & \multicolumn{2}{c}{\({RV}_{2}\)} & \multicolumn{2}{c}{\({rv}_{2}\)} \\  % & \(Date \)
