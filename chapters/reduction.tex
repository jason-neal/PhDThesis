...
% Possibly a future extent would be to model the th-ar lines, as well as the telluric lines, and 
...
\begin{figure}[h]
    ...
    \caption{A flat-field image for a detector \#1 before (left) and after(right) the non-linearity corrections are performed. A prefect detector would have all pixels in the flat-field equal to 1.}
...
\todo{Should I even include this section}
As part of the CRIRES calibration step there are images of {\thar} lamp spectra from 6 fibers.
We didn't use as there are known issues in the \nir. (but this is what they look like??)

See other section \sref{subsec:wavcalib}.
\missingfigure{\thar exposure showing the few lines for calibration?}
...
\caption{A Th-Ar calibration lamp frame for each detector, corrected from the dark current. \bf{Which is better?}}
    \label{fig:caliblamps}
\end{figure}

Scratches on the detector can cause a decrease in the correlation between {\thar} lines and the spectral template in ESO pipeline. This can be resolved by first applying a mask.
At the top and bottom there are also 2 meteorological fibers that can not be used for wavelength calibration as they pass through a different optical pathway.
...
To be able to extract the target spectra from the calibration and observation images a number of steps, some outlined above in \sref{subsec:nirreduction}, have to be performed in sequence. The software tools that performs this series of steps is refereed to as a \emph{pipeline}. Each stage in the pipeline performs a specific task (e.g. average dark frame creation or nod subtraction), then passing it to the next stage. Two different pipelines were available to reduce the CRIRES observations use in this work. The first is the standard CRIRES pipeline\footnote{\href{https://www.eso.org/sci/software/pipelines/}{https://www.eso.org/sci/software/pipelines/}}, available from ESO.
...
The experience gained in reducing CRIRES spectra enable participation in collaborations with other science cases. The DRACS pipeline was used to extract the spectra of two other targets. A target and a very brief aim of each science case is given below.
\begin{itemize}
\item Barnard's Star\footnote{Programme ID: 085.D-0161(A)}: The reduced {\nir} spectra of Barnard's Star was meant to extend the work of~\citet{andreasen_nearinfrared_2016} in deriving the spectroscopic parameters in the \nir. Unfortunately in the end the already reduced CRIRES-POP library was used instead in~\citet{Andreasen et al. (in prep.)}. \unfinished{reason}{This was because \ldots}
...
A common method for wavelength calibration that does not rely on Th-Ar lamps of CRIRES involves using the telluric lines present  to provide the wavelength solution ~\citep[e.g.][]{brogi_signature_2012,brogi_carbon_2014,dekok_detection_2013}. This is made possible by the use of high resolution spectra, in which the telluric lines are well resolved, as well as accurate spectral information of the atmosphere. 
...
\missingfigure{Example of coordinates and the fit.}
...
An briefly attempted wavelength calibration using the iterative method outlined in \cite{brogi_rotation_2016}. This involved generating set of wavelength solutions for the observed spectrum, cross-correlating each one against the telluric model. The solutions for the next iteration are obtained by refining the parameters from the solution with the highest correlation. This was not successful as we did not include a model or a mask of the stellar lines. The method worked well for \citet{brogi_rotation_2016} as they included templates for both star and telluric lines were observing in a wavelength domain in which there is a large number of strong and uniform stellar CO lines across the detector. Without adding any stellar masking the telluric lines would strongly correlate to the stellar lines, especially were there was blended lines. This method was abandoned, but may have been possible if a stellar mask was used.
...
Attempt of Brogi 2016 wavelength calibration method. Didn't perform well as we did not have many lines. They had a C02 comb of lines. 
...
In hind sight trying , especially for detector 2 trying to include the telluric spectra is 
...
All ground based astronomical observations pass though Earths atmosphere. The atmosphere becomes a spectral filter imprinting the absorption profile of the atmosphere onto the our the constituents of our atmosphere. To accurately recover the spectra of the observed target the removal of the absorption lines introduced by Earth's atmosphere is extremely important. The motion, changing composition and \todo{See solenes paper for matereial here}.
