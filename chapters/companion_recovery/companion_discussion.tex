...
\todo{Something I think that should be discussed in this section too is how the lines of the BD are not only small in absilute terms but smaller than the stellar counterparts. So the expectaions from the models are not only wrong but objectively too optimistic. }
...
To try and understand the recovered companions further investigation into the \textchisquared{} space was performed of the {HD30501} synthetic simulation.
...
\label{subsec:logg}
\todo{I think it is important to state that while the log g has a large impact on the data, there is no reason to believe that the log g were so badly determinated that they were the reason for the non-detection. The other potential issues seem far more likely.}
...
\citet{rajpurohit_exploring_2018} perform similar parameter determinations, using the recent {BT-Settl} models instead, exploring more lower temperature {M-dwarfs} than~\citet{passegger_carmenes_2018}, at which point dust clouds begin to form.
...
\todo{It is important to note that this is where your studies fit. So when trying to perform a model fitting at a \% fit ratio and smaller, you ran into the issues mentioned above.}
...
\todo{Finally, mention the issues with the spectra.}