...
\todo{This sentence is a bit overcomplicated due to: - repeated use of fitting;
    - focusing on what is not extracted.
    Please aim for a simpler sentence, and compelnet this with a second sentence if you want to stress what is not extracted.}
...
\todo{I would conclude the section then with the objective of this first-step simple fit, and what could be learned.}
...
The two spectra are scaled by their radius squared as the spectra are provided in units relative to the per unit area on the surface (see \cref{subsection-radius}). 
...
\todo{What do you mean by this? The age could still have an impact on the spectra of the different bodies. The way you wrote it implies that this methodology is fully insisitive to age. Why is that?}
...
\end{equation}
\todo{Ar these continuum fits the result of fitting the model and observations? Why do you fit the models too? A small explanation would be helpful.}
...
\todo{The danger of this is falling into a ocal minima. I think this amkes sense once we define a grid cloarse enough to be sure that we do not have local minima. We should think of a grid with a spacing comparable to the FWHM, or slightly smaller. Once it is defined that there is only one minima to explore at this level, we can go into a finer grid in the local minima region. }