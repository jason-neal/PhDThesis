...
Astrometry measures the precise position of the stars on the plane of the sky. The motion of a star with an exoplanet about its centre of mass can be observed in the periodic oscillating of position from its proper motion in the sky. Astrometry measures the stellar motion perpendicular to that probed by the RV method and is more sensitive to larger planetary separations, with the amplitude proportional to the companion/star mass ratio.

For a circular orbit and assuming the $M_p << M_{\star}$ the angle of the semi-major axis of the apparent orbital elipse, the amplitude of the astrometric signature ($theta$), is given by
\begin{equation} 
\theta = \frac{M_{p}}{M_{star}+ M_{p}} \approx \frac{1}{1} ..
\end{equation}
where, $M_p$ and $M_{\star}$ are the plant and stellar mass, $a$ is the semi-major axis and D is the distance from the observer to the system \citep{perryman__2011}\todo{finish}
 The amplitude of astrometric signal, decreases with distance, as the angles become smaller. This is unlike the RV and transit methods for which the amplitude is not affected by distance.

\todo{one confirmed, hd 176051b, muterspaugh 2010}
Although this method has not yet detected an exoplanet, it has found several Brown-dwarf companions \citep[e.g.][]{sahlmann_search_2011}.
The predicted astrometric variations for an exoplanet are at the level of sub-milliarcsecond.
