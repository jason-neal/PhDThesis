...
\end{equation}
where $\gamma$ is constant barycentre velocity of the system relative to the Sun, K is the velocity semi-amplitude, and \(omega\) is the argument of periastron. The true anomaly 
, \(\nu\) is a function of time, \(t\), orbital period \(P\), and the time of periastron passage, \(T_0\), and eccentricity.

The velocity amplitude $K$ of a star of mass $\Mstar$ due to a companion with mass $\Mp$ with orbital period $P$, eccentricity $e$ and inclination $i$ is~\citet[e.g.][]{cumming_lick_1999}:
...
The key exoplanet property determined by the {RV} technique is the companions mass, or \mtwosini.Where $i$ is the relative inclination of the orbital plane to the light of sight.
\todo{detail the other factors...}
...
M-dwarfs are inherently cooler and thus emit a majority of their stellar output in the near-infrared \reference{cite an M-dwarf paper?}, requiring dedicated \nir{} spectrographs such as {CARMENES}, {NIRPS}, and {SPIRou}.
...
Direct imaging is best suited to detect giant planets in wide orbits (>10\AU{}) around nearby stars.
The wide orbits are necessary to resolve the angular separation between the star and planet, and larger surface area which favour detection.
Young giants are favoured as they are still cooling and have higher thermal emission resulting in a higher constrast ratio to the host.
...
High-contrast adaptive optics instruments such as SHPEHE\citet{Beuzit_sphere_2008} on the VLT and GPI at~\citet{macintosh_gemini_2008}) are being used with several different techniques to observed targets closer to the star and with smaller contrasts, usually involving blocking or cancelling out the light from the star while keeping the planet \citep[e.g.[]{marois__2005, schmid__2005, sirbu 2017, mawet_2005, wang_2017}]

Along with exoplanets direct imaging has also been used to observe dust, rings and disks around young stars\textbf{reference}, and even image planets during formation \citep[e.g.][]{kraus 2012, sallum 2015}.
Combining direct images from different photometric bands can allow for the creation of low-resolution exoplanet spectra \citep[e.g.][]{kuzuhara__2013, zurlo__2015}.
...
The gravitation of a planet orbiting the lens star (if it exists) creates a distortion in the lens, leading to small caustics, deviations in the single lens microlensing light curve. An example of this is shown in \fref{fig:microlensing_example} where a lensing magnification of up to 3 is observed for {OGLE2005-BLG-390}~\citep{beaulieu_discovery_2006}. On the falling edge of the lensing event (and inset top right) there is a bump due to the presence of a 5.5\,\Mjup{} companion.
...
After the detection of exoplanets and the measurement of bulk properties, the next goal is detecting the atmosphere. Several techniques are being developed to available to this can give rise to the composition of the atmosphere, density, thickness
To enhance the characterization of exoplanets
Several of the main ones are detailed below.
...
radius changes with wavelength, extra absorbtion during transit can give molecular composition.
...
Such as the increase in atmospheric \ce{CO2} causing anthropamorphic climate change this requires 6\% change to \ce{CO2} line depths since 2000 Molecfit paper? There is also variation with airmass, which depends on the observation angle in the sky and changes as targets move across the sky during the night.
...
In this paper we apply two different techniques on FGK stars with BD companions with the aim to spectroscopically detect their companions. In \sref{sec:data} we present the observations and reduction process as well as the spectral models used in this work. In \sref{sec:specdiff} we explain the differential spectral technique and its applicability to these observations while in \sref{sec:results} we apply companion recovery using a \textchisquared{} approach. In \sref{sec:discussion} we discuss our results and in \sref{sec:conclusions} we present our conclusions.
