...
Echelle spectrographs are a special type of spectrograph, with a groove shape and orientation specifically for high reflection angles, and able to observe at a high spectral order (high $m$) to achieve a high dispersion and high resolution. \todo{I find that this paragraph is a bit too dense, with many different concepts; I would propose to expand a bit more on it, and make it less dense.}
...
In the mid-infrared (5--20\um) arsenic doped Silicon is used (\ce{Si}:\ce{As}). A summary of values for different material properties is given in \cref{tab:semiconductor_properties}.
...
\todo{I would add a note on how near-IR spectrographs are, by definition, temperature controled, and that has a very positive impact on the precision of RV measured with it.}
...
\todo{There is something missing that is very different between these spectrographs: simultaneous wavelength range. In this CRIRES is clearly the worst.}
...
{CARMENES} (Calar Alto high-Resolution search for M dwarfs with Exoearths with Near-infrared an optical \'Echelle Spectrographs) has been operating since 2016, performing a dedicated {RV} survey of \(\sim300\) late-type main-sequence stars with the goal of detecting low-mass planets in the habitable zone.
