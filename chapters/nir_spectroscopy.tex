...
For IR the technology of choice is {CMOS} ( -Metal-Oxide- ). The design of CMOS is vastly different to CDDs, with every pixel having an individual amplifier and ADC. Unlike CCDs the no charge to be lost transfering between pixels. While reading out the pixels on a CCD is a destructive process (one-time) the circuitry on in CMOS can measure and remeasure the pixel without affecting the value. Multiple samplings can be performed to average out the read noise of the amplifiers, referred to as Fowler sampling ().

Having individual amplifiers leads to non-linearities in gain between pixels.  As there are slight differences between each pixel. Where as all pixels in a CCDs are uniformly read by the same circuitry.

The design of CMOS detectors also requires circuity on the top-side of the pixels, effectively blocking parts of the photosensitive surface, reducing the detector efficiency.

CMOS detectors can be produced with many different crystals with specific band gaps and sensitive to different wavelengths.
