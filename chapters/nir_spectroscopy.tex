...
The index of refraction of a material is wavelength dependant as the angle of refraction will be different for each wavelength, causing dispersion, like a prism.
...
The resolving power, R is colloquially also referred to as the resolution\footnote{This document is no different.}. The higher the resolution (resolving power), R, the smaller the \(\delta\lambda\) that can be measured, leading to highly sampled spectral lines and more precise measurement.
...
\begin{figure}
    ...
    {NIRPS} is the smaller spectrograph. The cryostat's are shown in the open position and slide to enclose the instruments.
...
\begin{itemize}[\itemsep=0em] % Remove spacing on list.
