...
The loading is easily performed using Starfish's \textbf{GridTools}\todo{link from paper}, which can load the library spectra with a list of stellar parameters [\Teff{}, \logg{}, \feh{}, $[alpha/ce{He}]$].
The {PHOENIX-ACES} spectra are then converted from spectral energy distribution {} to photon flux {} by multiplying by the wavelength (multipliable constants ignored).
...
\begin{figure*}
    ...
    \caption[\textchisquared{} contour for companion recovery of a simulated Sun - M-dwarf binary.]{\textchisquared{} results for companion recovery of a simulated binary observation of a Sun-like star (\(\teffsub{1}=5\,800\)\K{}) with an M-dwarf companion (\(\teffsub{2}=4\,000\)\K{}).
...
\begin{figure*}
    ...
    \caption[\textchisquared{} contour for companion recovery of a simulated observation of {HD 211847}.]{Similar to \cref{fig:Mdwarf_contours}, \textchisquared{} results for companion recovery of a simulated binary observation similar to {HD 211847}, (\(\teffsub{1} = 5\,800\)\K{}, \(\teffsub{2}=3\,200\)\K{}).
    The top right plot shows the application of a single component model (\(C^1\)) while the other three are using a binary model (\(C^2\)).
