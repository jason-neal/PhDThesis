...
Take 1 sampling width at centre of band - > scale to specific snr require.
...
Care needs to be taken to optimize the code. The original code used in \citet{figueira_radial_2016} was very slow, taking around 2 hours per simulation, this led to multiple weeks of processing time to compute the precision's for the original paper.
...
One of the key insights from \eref{} is that radial velocity error is inversely proportional slope of the spectra, with sharper lines producing a smaller RV error. In numerically computing the RV precision, the result is dependent on method used to numerically calculate the gradient or slope of the spectral. 
...
The original method used in \citet{figueira_radial_2016} used the simplest method to calculate the slope of the lines. \(\rm\frac{rise}{run}\) forward   

There are three different varieties,  
\[f'(x) = \lim_{h \to 0} \frac{f(x+h)-f(x)}{h}, f'(x) = \lim_{h \to 0} \frac{f(x+\frac{1}{2}h)-f(x-\frac{1}{2}h)}{h}, f'(x) = \lim_{h \to 0} \frac{f(x)-f(x-h)}{h}\], called the forwards, central and backwards respectively
The first of which should be familiar from introductory differentiation courses.
The order of uncertainty  on the forward/backwards finite difference is $\O(h)$ while for the central difference it is $\O(h^2)$ \citep{quarteroni_numerical_2000}. As the difference between points (h) is small the central difference gives a more precise value for the gradient at each point.
...
At each pixel the numerical derivative is evaluated to the average slope between itself and the following point and is an approximation to the derivative.


There are other numerical methods to calculate the derivative. The \emph{gradient}\footnote{\href{https://docs.scipy.org/doc/numpy/reference/generated/numpy.gradient.html\#id1}{https://docs.scipy.org/doc/numpy/reference/generated/numpy.gradient.html\#id1}}  function in  Numpy is implemented central differences in the interior points, accurate to the second order differential, and first order accurate one-sided (forward or backwards) differences at the boundaries...
...
\begin{figure}
    \centering
    \begin{tabular}
    \includegraphics[width=0.7\linewidth]{figures/information-content/spectra_gradient_example1} \includegraphics[width=0.7\linewidth]{figures/information-content/spectra_gradient_example2}\\
    \end{tabular}
    \caption{}
