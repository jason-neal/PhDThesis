...
The calculation of RV precision is susceptible to difference in numerical gradients.
Changing the spectral derivative algorithm from using  the simple dy/dx to using np.gradient()\footnote{\href{https://docs.scipy.org/doc/numpy/reference/generated/numpy.gradient.html\#id1}{https://docs.scipy.org/doc/numpy/reference/generated/numpy.gradient.html\#id1}} which preforms a second order differencing
\citet{quarteroni_numerical_2000}  reference from np.gradient
...
\(F'[i] = \frac{f([i+1) - F(i-1)}{\lambda(i+1)-\lambda(i-1)}\)    where the ends are handles using forwards or backwards difference quotients.   

These higher order derivatives give a better estimate of the derivative ()

Finally, the central difference is given by

$\delta _{h}[f](x)=f(x+{\tfrac {1}{2}}h)-f(x-{\tfrac {1}{2}}h)~.$

Numpy  Gradient notes:
