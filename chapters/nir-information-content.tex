...
The fundamental radial velocity precision of {M-dwarf} spectra attainable at different wavelength regions calculated in~\citet{figueira_radial_2016} was used to influence some design choices of {SPIRou}, {NIRPS} specifically.\todo{Pedro it was the wavlength selects to focus on correct??}
...
Rearranging \cref{eq:intensitychange} for \(\delta \lambda\) and combining it with \cref{eq:dopplershift}, the Doppler shift of then becomes:
\begin{equation}
...
\Cref{eq:delta_v_i} provides a separate measurement of the radial velocity shift for every pixel, $i$, in the spectrum.
...
The individual dispersion of the velocity measurement \(\delta V_{\rms}(i)\) is the dispersion that would result from several measurements of the reference spectrum all with the same Doppler shift (e.g.\ zero). \Cref{eq:delta_v_i} thus becomes:
\begin{equation}
...
The {\red{}\(RV_{\rms}\)} is calculated for each section and then the results are combined as the weighted error in {\red{} \cref{XXX}} \todo{weighted sum equation}.
...
As shown in \cref{eq:snrequation} the {RV} precision is inversely proportional to the \snr{} level.
