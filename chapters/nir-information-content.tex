...
The important quantity for RV measurements is not only the velocity value itself but the dispersion on the measured velocity or the precision, \(\delta V_{RMS}\). From \eref{eq:weights} the dispersion is given by:
...
\citet{Connes1985} show that the optimal weights for photon limited noise is given by \todo{finish this}
...
\todo{Update the legend in figure  \fref{fig:gradients}}
\begin{figure}
    \centering
    \begin{tabular}{cc}
    ...
    \end{tabular}
    \caption{Visualization of computing the numerical gradient. Top: The two spectral regions of a stellar spectrum the left is a large absorption line while the right is short lines near the continuum (~1). Bottom: The numerical gradients of the spectra shown in the top panels the original FFD method is given in blue dots while numpy gradient is shown by green circles. The orange stars are the FFD version shifted to the mid-points between pixels for illustrative purposes.}
    \label{fig:gradients}
\end{figure}
...
Another change made to the software is in the application of the masking function, and the treatment of telluric lines. As suggested in~\cite{Connes1985} and~\cite{bouchy_fundamental_2001} a custom masking function can be applied to the individual pixel weights in \eref{eq:optimal_weight}, such as:
...
\begin{itemize}
 \item Condition \#1:
     Condition \#1 is simplest case in which telluric lines are not considered. All pixels are considered equal with no pixel weighting the masking function would be all one.
...
\item Condition \#2:
 ...
 where \(T(i)\) is the telluric transmission spectrum (between 0 and 1) and \(\tau\) is the depth limit to mask, e.g.\ for a telluric depth limit of 2\%, \(\tau\) would be 0.98.
  \item Condition \#3:
  Condition \#3 assumes perfect telluric correction by dividing the flux variance by square of the transmission spectrum \reference{cite the equation when I refer to it}. On rearranging this is equivalent to applying a mask of the form,

\end{itemize}

Figueira et al. 2016 chose to use this mask in a round about way.
They split the spectra on the individual chunks left over (essentially removing the \todo{finish this phrase})
...
Precisions were provided for SNR of 100 relative to the J, H bands as well as to each band individually. Artigua 2018. (private communication 2018)\todo{Check how to cite priv communication properly} suggested the truly relevant value is the SNR in H band for NIRPS radial velocities.
