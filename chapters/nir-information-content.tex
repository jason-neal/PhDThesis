...
where \(\Sigma A_0(i) = {N}_{{e}^{-}}\) is considered to be the total number of photo-electrons \({N}_{{e}^{-}}\) counted in the spectral range considered.\todo{explain snr}{where the \({SNR}=\sqrt{N_{e^-}}\)for large N}
...
Stellar rotation has the affect of broadening spectral lines as the different portions of the stellar surface have a variation of radial velocity between \(\pm v\sini\). 
...
\todo{top veiw diagram of rotation?}
...
\begin{figure}
    ...
    \includegraphics{} &  % Figueria plot
    \includegraphics{} \\ % Eniric plot
    \end{tabular}
    \caption{Caption}
    \label{fig:my_label}
\end{figure}
