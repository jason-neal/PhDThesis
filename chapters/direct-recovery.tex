...
The first observation easily observed is that RV curves come in many different flavours(shapes). This variations in shape arise from the different orbital parameters of each target (provided in \tref{tab:orbitparams}). In the left hand plots, in which a full cycle is shown the different shapes are created from the eccentricity, $e$, and argument of periapsis, $\omega$. In the right hand panels for which a fixed time period is shown the parameter affecting the image shown is the orbital period of the companion. Specifically the ratio of Period 89 to the orbital period determines what fraction (or multiples) of the orbit to display.
