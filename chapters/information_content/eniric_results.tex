...
In comparing the bottom two panels between the {PHOENIX-ACES} and {BT-Settl} models, there is only small differences, most identifiable in the Condition~\#2 values of the \emph{J}-band.
...
With the ability to explore a wider range of parameters the range of was {PHOENIX-ACES} models extended to explore the affect of \Logg{} and \feh{} of the {PHOENIX-ACES} models on the relative {RV} precision.
Remember that \Logg{} is a measure of the stellar surface gravity, the gravitational acceleration at the equator expressed in {cgs} units of \si{\centi\metre\per\second\squared} then taking the logarithm base-10.
The surface gravity \(g \propto \frac{M}{R^{2}}\) so a larger \Logg{} values corresponds to stars with smaller radii.
While the metallicity, \feh{}, is a measure of the abundance of elements heavier than Hydrogen and Helium.
It is often measured as the ratio of Iron to Hydrogen relative to the Sun.
That is a \feh{}=0 has the same metal ratio as the Sun a positive \feh{} has more metals, and a negative \feh{} has less metals.
...
The spectral quality factor variations for the {M-dwarf}s {M0}, {M3}, {M6}, {M9}, with a broadening of R=100\,000 and $\vsini=1.0$\kmps{} across the \nir{} bands is shown \cref{fig:logg_metalicity_deviations}.
The top row shows the quality for model spectra with a fixed \Logg=4.5 but the have a variable \feh{} between -1.0 to 1.0.
...
While {M6} and {M9} both increase with increasing \Logg{} so overall there is a divergence in the spectral quality at a larger \Logg{} in both bands.
...
For the {M0} and {M3} spectra the quality is mostly stays under 10\,000, apart from a four points in the Z and \emph{Y}-bands at high \feh{}.
For the cooler {M6} and {M9} quality values however there is a large contrast between the Z and \emph{Y}-bands, which have a much higher quality, and the other three bands which have a similar quality level.
...
They show flux as a function of wavelength in the \emph{Z}-band (top) and \emph{J}-band (bottom) for a \Vsini{}=1.0\kmps{} and spectral types {M0}, {M3}, {M6}, and {M9} (\textit{top} to \textit{bottom} panels) spectra, when seen at a resolution at 100\,000
...
\begin{figure}
    ...
    \caption{Flux as a function of wavelength in the \emph{Z}-band (top) and \emph{J}-band (bottom) for the \Vsini{}=1.0\kmps{} and spectral types {M0}, {M3}, {M6}, and {M9} (\textit{top} to \textit{bottom} panels) spectra, when seen at a resolution at 100\,000.
...
With the possibility to simply compute the quality and precision of a large range of synthetic spectral parameters.
A comparison of spectral quality over all synthetic spectra or just all in the M-dwarf temperature range, may help solidify or extend the trends observed here.
...
The result tables, along with the command line incantation are detailed in \cref{appendix:nir_prec_amendment}.
