...
This kernel arises from integrating the rotational velocity profile across the surface of the stellar disk as a series of strips with different Doppler shifts. As such the rotation kernel is bounded in the range $[-\Delta\lambda_L, \Delta\lambda_{L}]$ from the line centre.
...
It only provides a small correction to the Gaussian profile of a standard spectrograph, and only necessary if one is resolution-limited by the slit. % (priv. comm. Pefdro Figueira)
