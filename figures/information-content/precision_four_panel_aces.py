...
def load_dataframe(filename):
    ...
    df["[Fe/H]"] = df["feh"].astype(float)
    df["Alpha"] = df.alpha.astype(float)
    df["vsini"] = df.vsini.astype(float)
    df["Sampling"] = df.sampling.astype(float)
    df["Cond. 1"] = df["cond1"].astype(float)
    df["Cond. 2"] = df["cond2"].astype(float)
    df["Cond. 3"] = df["cond3"].astype(float)
    df["correct flag"] = df["correctflag"].astype(bool)
...
def plot_precision(
    ...
    for ii, temp in enumerate(teffs):
        ...
        ax[ii].plot(
            df_ii_60k.index, df_ii_60k["Cond. 1"].values, color="b", linestyle="--", linewidth=1,
        )  # lim
        ax[ii].plot(
            df_ii_80k.index, df_ii_80k["Cond. 1"].values, color="g", linestyle="--", linewidth=1
        )  # lim
        ax[ii].plot(
            df_ii_100k.index, df_ii_100k["Cond. 1"].values, color="r", linestyle="--", linewidth=1
        )  # lim
    ...
    for jj in range(4):
        ax[jj].text(0, ymax, "{} K".format(teffs[jj]), size=13)
        ...
        ax[jj].set_yticks([0,10,20,30,40,50])
...
def cumulative_plot(
    ...
    for ii, temp in enumerate(teffs):
        ...
        ax[ii].plot(
            df_ii_60k.index, df_ii_60k["Cond. 1"].values, color="b", linestyle="--", alpha=0.8
        )  # lim
        ax[ii].plot(
            df_ii_80k.index, df_ii_80k["Cond. 1"].values, color="g", linestyle="--", alpha=0.8
        )  # lim
        ax[ii].plot(
            df_ii_100k.index, df_ii_100k["Cond. 1"].values, color="r", linestyle="--", alpha=0.8
        )  # lim
    ...
    for jj in range(4):
        ...
        ax[jj].set_xlim(-0.5, band_size - 0.5)
        ax[jj].tick_params(axis="both", which="major", labelsize=11)

        # ticks and labels
        if (jj == 2) or (ii == 3):
            ax[jj].set_xlabel("Bands", fontsize=11)
            if full_cum:
...
if __name__ == "__main__":
    ...
    #)
    precision_file = os.path.join("precision", "aces_prec.csv"
    )
