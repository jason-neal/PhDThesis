...
def plot_precision(
    ...
    for ii, temp in enumerate(teffs):
        ...
        ax[ii].plot(
            df_ii_60k.index, df_ii_60k["Cond. 1"].values, color="b", linestyle="--", linewidth=1, alpha=0.8
        )  # lim
        ax[ii].plot(
            df_ii_80k.index, df_ii_80k["Cond. 1"].values, color="g", linestyle="--", linewidth=1, alpha=0.8
        )  # lim
        ax[ii].plot(
            df_ii_100k.index, df_ii_100k["Cond. 1"].values, color="r", linestyle="--", linewidth=1, alpha=0.8
        )  # lim
    ...
    fig.savefig("plots/precision_logg{0}_feh_{1}_btsettl.pdf".format(logg, fe_h))
    fig.savefig("plots/precision_logg{0}_feh_{1}_btsettl.png".format(logg, fe_h), dpi=400)
...
if __name__ == "__main__":
    ...
    #)
    precision_file = os.path.join("precision", "btsettl.csv"
    )
