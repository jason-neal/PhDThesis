...
\newcommand{\Communication}[8]{
    \begin{mdframed}[style=Publication, linecolor=#1,singleextra={\fill[#1] (O) rectangle ([xshift=20pt]P-|O);
        ...
        \normalsize
        \begin{tabular}{>{\normalsize\bfseries}p{.11\linewidth} >{\normalsize}p{.46\linewidth} >{\normalsize\bfseries}p{.07\linewidth} >{\normalsize}p{.2\linewidth}}
            \multicolumn{4}{>{\bfseries\Large\centering\color{#1}}p{.92\linewidth}}{#2\newline}    \\[-2em]
            \midrule    \\[-.5em]
            Location:    &     {#4} & Date:    &    {#3}\\
            Website:    &    \multicolumn{3}{>{\normalsize}p{.83\linewidth}}{\url{#5}}\\
            %        Points:    &    {#6} & &    \\
           \midrule\\
        \end{tabular}\\
...
\newcommand{\Other}[8][MainColor]{
    \begin{mdframed}[style=Publication, linecolor=#1,singleextra={\fill[#1] (O) rectangle ([xshift=20pt]P-|O);
        ...
        \normalsize
        \begin{tabular}{>{\normalsize\bfseries}p{.11\linewidth} >{\normalsize}p{.46\linewidth} >{\normalsize\bfseries}p{.07\linewidth} >{\normalsize}p{.2\linewidth}}
            \multicolumn{4}{>{\bfseries\Large\centering\color{#1}}p{.92\linewidth}}{#3\newline}    \\[-2em]
            \midrule\\[-.5em]
            Location:    &     {#4} & Date:    &    {#5}\\
            Website:    &    \multicolumn{3}{>{\normalsize}p{.83\linewidth}}{\url{#6}}\\
            %        Points:    &    {#7} & &    \\
            \midrule\\
           %# \hline\\
        \end{tabular}\\
        \hspace{20pt}\textbf{Description: } \\
        {#8}
    \end{mdframed}%\vspace{3em}
}
