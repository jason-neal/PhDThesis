...
\tref{tab:rv_aces_btsettl} shows the precision results for the same M-dwarfs analysed in \citep{figueira_radial_2016}.
...
There is small difference in Conditions 1 and 3 from the change in numerical differentiation implemented (see \sref{sec:numerical_gradient}).
The values for Condition 2 however, are completely different due to the implementation error in the telluric masking discovered (see \sref{subsec:condition_two_bug}).
...
The values are provided in \tref{tab:spirou_precisions} and can be generated with the following code; note the change in the \emph{ref\_band} parameter.
...
Comparing the precision values between \tref{tab:rv_aces_btsettl} and \tref{tab:spirou_precisions}, it has the effect of decreasing the $\sigma_{RV}$ (increasing precision) in the \emph{H}- and \emph{K}-bands while increasing the $\sigma_{RV}$ (decreasing precision) in the \emph{Z}- and \emph{y}-bands.
The RV precision for the \emph{J}-band remains unchanged, so is not included in \tref{tab:spirou_precisions}.
...
The results for the {NIRPS} precision relative to the \emph{J}-band are given in \tref{tab:nirps_precisions}, and can be reproduced with the following code.
